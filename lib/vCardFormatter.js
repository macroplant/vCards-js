...
(function vCardFormatter() {
	...
	module.exports = {
		...
		getFormattedString: function(vCard) {
			...
			vCard.emailAddresses.forEach(
				function(entry) {
          const { type, address } = entry;
          const formattedType = type.toUpperCase();
					if (majorVersion >= 4) {
						formattedVCardString += `EMAIL${encodingPrefix};type=${formattedType}:${e(address)}${nl()}`;
					} else if (majorVersion >= 3 && majorVersion < 4) {
						formattedVCardString += `EMAIL${encodingPrefix};type=${formattedType},INTERNET:${e(address)}${nl()}`;
					} else {
						formattedVCardString += `EMAIL${encodingPrefix};${formattedType};INTERNET:${e(address)}${nl()}`;
					}
				}
			);
			...
			vCard.phoneNumbers.forEach(
				function(entry){
          const { type, number } = entry;
					if (majorVersion >= 4) {
						formattedVCardString += `TEL;VALUE=uri;TYPE="voice,${type.toLowerCase()}":tel:${e(number)}${nl()}`;
					} else {
						formattedVCardString += `TEL;TYPE=${type.toUpperCase()}:${e(number)}${nl()}`;
					}
				}
			);

			vCard.faxNumbers.forEach(
				function(entry) {
          const { type, number } = entry;
					if (majorVersion >= 4) {
						formattedVCardString += `TEL;VALUE=uri;TYPE="fax,${type.toLowerCase()}":tel:${e(number)}${nl()}`;

					} else {
						formattedVCardString += `TEL;TYPE=${type.toUpperCase()},FAX:${e(number)}${nl()}`;
					}
				}
			);

			vCard.mailingAddresses.forEach(
				function(address) {
      ...
      vCard.urls.forEach(
        function(entry) {
          ...
          formattedVCardString += `URL;type=${type.toUpperCase()}${encodingPrefix}:${e(url)}${nl()}`;
        }
      )
